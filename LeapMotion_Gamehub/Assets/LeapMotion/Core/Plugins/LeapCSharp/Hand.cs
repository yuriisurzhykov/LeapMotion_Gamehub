...
namespace Leap
{
    ...
    public class Hand : IEquatable<Hand>
    {
        ...
        public Finger Finger(Finger.FingerType type)
        {
            for (int i = Fingers.Count; i-- != 0;)
            {
                if (Fingers[i].Type == type)
                {
                    return Fingers[i];
                }
            }
        ...
        {
            Vector index = Finger(Leap.Finger.FingerType.TYPE_INDEX).Direction;
            Vector ring = Finger(Leap.Finger.FingerType.TYPE_RING).Direction;
            Vector middle = Finger(Leap.Finger.FingerType.TYPE_MIDDLE).Direction;
            Vector pinky = Finger(Leap.Finger.FingerType.TYPE_PINKY).Direction;
            Vector thumb = Finger(Leap.Finger.FingerType.TYPE_THUMB).Direction;
            if (index.x < 0 && index.y < 0 && index.z > 0 &&
                ring.x < 0 && ring.y < 0 && ring.z > 0 &&
                middle.x < 0 && middle.y < 0 && middle.z > 0 &&
                pinky.x < 0 && pinky.y < 0 && pinky.z > 0 &&
                thumb.x > 0 && thumb.y > 0 && thumb.z < 0)
            {
                return true;
            }
            else
